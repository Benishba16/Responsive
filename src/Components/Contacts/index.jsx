...
import { getContact, deleteContact } from '../../Constants/contactAction/actions';
...
import './style.scss';
import ImportContentContect from "../ImportContactContent";
...
const Contacts = (props) => {
  ...
  const [onClosed, setClosed] = useState(false)
  ...
  <div className="contacts">
    <section className="contacts--header d-flex justify-content-between align-items-center">
      ...
      <div className="pr-3">
        <span className="btn btn-secondary mx-2 my-3" onClick={() => setClosed(!onClosed)}>Import Contacts</span>
        ...
        {actionValues.length > 0 && <span className="btn btn-primary mx-2 my-3" onClick={() => {setActionValues([]); props.deleteContact(actionValues)}}>Delete</span>}
        <Dialog header={actionValues.length === 1 ? "Update Contact" : "Add Contact"} handleClose={handleClose} isClosed={isClosed}>
          <AddContact {...props} setClose={setClose} setActionValues={setActionValues} actionValues={actionValues} />
        </Dialog>
      </div>
    </section>
    ...
    {!!structure && 
      ...
      {
        onClosed &&
        <Dialog1 header="Import Contact" handleCloseDialog={handleCloseDialog} onClosed={onClosed}>
          <ImportContentContect handleCloseDialog={handleCloseDialog}/>
        </Dialog1>
      }
  </div>
)};
